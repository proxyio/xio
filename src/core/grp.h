...
static inline void grp_init(grp_t *grp) {
    spin_init(&(grp)->lock);
...
static inline void grp_destroy(grp_t *grp) {
    struct role *r, *n;		

    spin_destroy(&(grp)->lock);	
    list_for_each_role_safe(r, n, &(grp)->rcver_head) {	
	r_destroy(r);					
	mem_free(r, sizeof(*r));			
    }							
    list_for_each_role_safe(r, n, &(grp)->snder_head) {	
	r_destroy(r);					
	mem_free(r, sizeof(*r));			
    }							
}

static inline void grp_add(grp_t *grp, struct role *r) {
    struct list_head *__head =
	IS_RCVER(r) ? &grp->rcver_head : &grp->snder_head;
...
static inline void grp_del(grp_t *grp, struct role *r) {
    grp_lock(grp);				
    grp->rsize--;				
...
static inline void __grp_walk(grp_t *grp, walkfn f, void *args,
